...
/**
 * LoggerSingleton class provides a centralized logging mechanism for the DevFlex VS Code extension.
 */
export class LoggerSingleton extends vscode.Disposable {
	...
	private constructor() {
		...
		this.outputChannel = vscode.window.createOutputChannel('DevFlex', {
			log: true,
		});
			...
			.then(async (selection) => {
				if (selection === 'Open Docs') {
					vscode.env.openExternal(
						vscode.Uri.parse('https://DevFlex.ai/installation'),
					);
				}
			});
	}
