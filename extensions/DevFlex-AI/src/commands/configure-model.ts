...
import { ICustomDialogInputOptions, ICustomDialogOutputResult } from '../../../../src/vscode-dts/DevFlex';
...
const addNewConfiguration = async (): Promise<void> => {
	...
	const provider = await vscode.window.showQuickPick(
		...
		ignoreFocusOut: true,
		title: 'DevFlex: Select the chat model provider',
	}
	);
	if (!provider) { return; }
...
const deleteConfirmationDialog = async (configId: string): Promise<void> => {
	...
	const confirmNickname = await vscode.window.showInputBox({
		title: 'DevFlex: Delete language model provider',
		ignoreFocusOut: true,
...
const completionsConfiguration = async (): Promise<void> => {
	try {
		...
		const dialogOptions: ICustomDialogInputOptions = {
			...
			custom: { dialogId: 'DevFlex-model-config' },
			inputs: [
				{
		...
		const response = await vscode.commands.executeCommand<ICustomDialogOutputResult>(
			'DevFlex.custom.dialog.show', dialogOptions);
...
/**
 * Handles the configuration process for the DevFlex extension.
 */
const handler = async () => {
	...
	quickPick.title = 'DevFlex: Configure the Language Model Provider';
	...
	quickPick.onDidChangeSelection(async (selection) => {
		...
		if (selection[0].label === 'Add Chat Model Config') {
			...
			vscode.commands.executeCommand('DevFlex.usagePreferences');
		} else {
...
/**
 * Registers the 'DevFlex.configureModel' command with the Visual Studio Code extension context.
 */
export const registerConfigureModelCommand = () => {
	registerDisposable(vscode.commands.registerCommand('DevFlex.configureModel', handler));
	logger.info('Command `DevFlex.configureModel` registered');
};
