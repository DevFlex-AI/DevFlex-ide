...
const checkInternetConnection = async () => {
	try {
		await fetch(corsEnableUrl('https://DevFlex.ai'), { method: 'HEAD' });
		return true;
	} catch (error) {
		logger.error(error);
		return false;
...
/**
 * Handles the internet connection check for the DevFlex extension.
 */
const handler = async () => {
	...
	if (githubSession && !isAgentsActivated) {
		...
		await vscode.window.withProgress({
			...
			if (!isConnected) {
				vscode.window.showErrorMessage(
					'DevFlex: No internet connection', { modal: true }, 'Retry'
				).then((selection) => {
					if (selection === 'Retry') {
						vscode.commands.executeCommand('DevFlex.checkInternetConnection');
					}
				});
		...
		vscode.window
			.showInformationMessage(
				'Please sign in to your GitHub account to start using DevFlex',
				'Sign in to Chat',
			)
			.then((selection) => {
				if (selection === 'Sign in to Chat') {
					vscode.commands.executeCommand('DevFlex.github.signin');
				}
			});
	}
};
...
/**
 * Registers the internet connection check command for the DevFlex extension.
 */
export const registerCheckInternetConnectionCommand = () => {
	registerDisposable(vscode.commands.registerCommand('DevFlex.checkInternetConnection', handler));
	registerDisposable(vscode.authentication.onDidChangeSessions(handler));
	logger.info('Command `DevFlex.checkInternetConnection` registered');
};
