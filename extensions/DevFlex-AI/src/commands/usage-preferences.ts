...
/**
 * Handles the model configuration process for the DevFlex extension.
 */
...
const handler = async () => {
	...
	const pickedLocation = await vscode.window.showQuickPick(
		...
		{
			title: 'DevFlex: Modify Model Usage Preferences',
			ignoreFocusOut: true,
			canPickMany: false,
...
/**
 * Registers the model configuration command for the DevFlex extension.
 */
export const registerUsagePreferencesCommand = () => {
	registerDisposable(vscode.commands.registerCommand('DevFlex.usagePreferences', handler));
	logger.info('Command `DevFlex.usagePreferences` registered');
};
