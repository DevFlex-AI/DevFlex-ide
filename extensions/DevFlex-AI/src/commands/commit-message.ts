...
/**
 * Registers the `DevFlex.git.generateCommitMessage` command with the Visual Studio Code extension context.
 */
export const registerCommitMessageCommand = () => {
	registerDisposable(vscode.commands.registerCommand('DevFlex.git.generateCommitMessage', handler));
	logger.info('Command `DevFlex.git.generateCommitMessage` registered');
};
