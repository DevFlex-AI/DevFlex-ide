...
const handler = async () => {
	vscode.window.showInformationMessage(
		...
		if (selection === 'Download Desktop IDE') {
			vscode.env.openExternal(vscode.Uri.parse('https://DevFlex.ai'));
		}
	});
};
...
export const registerVfsInfoMessageCommand = () => {
	registerDisposable(vscode.commands.registerCommand('DevFlex.vfs.info.message', handler));
	logger.info('Command `DevFlex.vfs.info.message` registered');
};
