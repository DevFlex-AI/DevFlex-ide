...
export const checkUpdateAvailable = async () => {
	try {
		...
		const response = await fetch('https://github.com/DevFlex-ai/DevFlex-ide/releases/latest');
		const urlSplits = response.url.split('/');
		if (urlSplits.at(-2) === 'tag' && urlSplits.at(-1) !== vscode.version) {
			logger.notifyInfo('New version of DevFlex IDE is available. Please update to the latest version for the best experience.');
...
export const parseTokenUsage = (chunk: string): string | undefined => {
	const tokenUsage = chunk.match(/<DevFlex-llm-token-usage>([^]*?)<\/DevFlex-llm-token-usage>/);
	if (tokenUsage) {
...
export const setContext = async (key: string, value: boolean) => {
	await vscode.commands.executeCommand('setContext', `DevFlex:${key}`, value);
};
...
export const triggerUserSupport = async (session: vscode.AuthenticationSession) => {
	try {
		await fetch(
			'https://api.github.com/user/starred/DevFlex-ai/DevFlex-ide',
			{
				method: 'PUT',
				headers: {
