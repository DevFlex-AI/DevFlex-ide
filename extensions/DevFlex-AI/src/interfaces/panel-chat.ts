...
/**
 * Provides welcome messages and sample questions for the DevFlex chat panel.
 */
const welcomeMessageProvider: vscode.ChatWelcomeMessageProvider = {
	...
	provideSampleQuestions: async () => {
		return [{ prompt: '/help - Get help with DevFlex commands' }];
	}
};
...
const chatRequestHandler: vscode.ChatExtendedRequestHandler = async (request, context, response, token) => {
	try {
		...
		const returnTokenUsage = vscode.workspace.getConfiguration().get<boolean>('DevFlex.panelChat.showTokenUsage');
		...
		response.button({ command: 'DevFlex.viewLogs', title: 'View Logs' });
		return {
...
/**
 * Registers a new panel chat participant for the DevFlex extension.
 */
export const register = async () => {
	...
	chatParticipant = vscode.chat.createChatParticipant('DevFlex.panel.default', chatRequestHandler);
