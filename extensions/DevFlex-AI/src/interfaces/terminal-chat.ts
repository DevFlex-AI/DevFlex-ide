...
const chatRequestHandler: vscode.ChatExtendedRequestHandler = async (request, context, response, token) => {
	try {
		...
		const returnTokenUsage = vscode.workspace.getConfiguration().get<boolean>('DevFlex.terminal.showTokenUsage');
...
export const register = async () => {
	...
	chatParticipant = vscode.chat.createChatParticipant('DevFlex.terminal.session', chatRequestHandler);

	// Set up welcome message provider
	chatParticipant.welcomeMessageProvider = {
		provideWelcomeMessage: async () => {
			const message = new vscode.MarkdownString('Welcome to the DevFlex AI terminal chat!');
			return { icon: new vscode.ThemeIcon('terminal'), title: 'Ask DevFlex', message };
		}
	};
