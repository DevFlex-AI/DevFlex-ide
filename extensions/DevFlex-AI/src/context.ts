...
// The extension context for the DevFlex extension
...
export const modelConfigs = {
	...
	get: <T1 extends IModelConfig>(key: string): T1 | undefined => {
		return getExtensionContext().globalState.get<T1>(`DevFlex.modelProvider.${key}`);
	},
	...
	update: async <T1 extends IModelConfig>(key: string, value: T1 | undefined): Promise<void> => {
		return getExtensionContext().globalState.update(`DevFlex.modelProvider.${key}`, value);
	},
	...
	list: (): string[] => {
		...
		for (const item of getExtensionContext().globalState.keys()) {
			if (item.startsWith('DevFlex.modelProvider')) {
				result.push(item.replace('DevFlex.modelProvider.', ''));
			}
		}
		return result;
	}
