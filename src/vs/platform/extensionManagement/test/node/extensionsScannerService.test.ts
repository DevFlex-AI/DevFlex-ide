...
class ExtensionsScannerService extends AbstractExtensionsScannerService implements IExtensionsScannerService {
	...
	constructor(
		...
		super(
			...
			joinPath(nativeEnvironmentService.userHome, '.DevFlex-ide-dev', 'extensions', 'control.json'),
...
suite('NativeExtensionsScanerService Test', () => {
	...
	test('scan system extensions include additional builtin extensions', async () => {
		...
		await instantiationService.get(IFileService).writeFile(joinPath(instantiationService.get(INativeEnvironmentService).userHome, '.DevFlex-ide-dev', 'extensions', 'control.json'), VSBuffer.fromString(JSON.stringify({ 'pub.name2': 'disabled', 'pub.name': extensionLocation.fsPath })));
		const testObject: IExtensionsScannerService = disposables.add(instantiationService.createInstance(ExtensionsScannerService));
